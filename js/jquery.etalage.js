...
;(function($){
	...
	$.fn.etalage = function(options){
		...
		if(this.length == 0) {
                ...
                var autotimer = false;
                var contBack = el.get(0).outerHTML; //container Backup
                var contParent = el.parent(); //container parent for reinsertion
                var $container = el;
...
// ACTIONS
                ...
                el.reload = function (settings) {
                    ...
                    $container.remove();
                    clearInterval(autotimer);
                    contParent.prepend(contBack);
                    $container = $('#etalage', contParent);
                   
                   var time = setTimeout (init,2000);
